    ...
    static async start(opts) {
        try {
            ...
            if (cd.cm.dataDir &&
                cd.setgid !== undefined &&
                cd.setuid !== undefined) {
                console.log("cd data dir", cd.cm.dataDir);
    ...
    processConfig() {
        return new Promise((resolve) => {
            ...
            for (let config of confList) {
                ...
                // save gid / uid
                if (config.setuid) this.setuid = config.setuid;
                if (config.setgid) this.setgid = config.setgid;
    ...
    installComponents(componentList) {
        ...
        return new Promise((resolve, reject) => {
            ...
            npm.load({
                ...
                // console.log ("npm", npm);
                // npm.log.disableColor();

                npm.commands.install(packageList, (err2, results) => {
                    if (err2) reject(err2);
                    process.stdout.clearLine();
                    process.stderr.clearLine();

                    // results have path that package was installed to
                    // save that path for each of the packages
                    var resultMap = new Map();
                    // package name with @version
                    results.forEach((result) => resultMap.set(result[0], result[1]));
                    // package name doesn't include @version
                    results.forEach((result) => resultMap.set(result[0].substring(0, result[0].indexOf("@")), result[1]));

                    componentList.forEach((component) => {
                        component.packagePath = resultMap.get(component.packageName);
                        if (!component.packagePath) {
                            throw new Error("couldn't get package path for: " + component.package);
                        }
                    });
